    ...
    if len(call_urel) == 3:
        ...
        try:
            ...
            with conn.cursor() as cur:
                ...
                result = cur.fetchall()
                print('post /getwalletsyncdata: get blockCount: {} topHeight:{}'.format(blockCount, topHeight))
                end = time.time()
                print('Query time to fetch blockHashes Data before query tx: {}s'.format(end-start))
                blockList = await post_getwalletsyncdata(result)
                print('Query time from blockList: {}s'.format(time.time()-end))
